...
class WebiCartItems extends HTMLElement {
  constructor() {
    ...
    this.currentItemCount = Array.from(
      ...
      (total, webiquantityInput) => total + parseInt(webiquantityInput.value),
      0
    )
  ...
  updateLiveRegions(line, itemCount) {
    if (this.currentItemCount === itemCount) {
      document
        .getElementById(`Wb-Line-item-error-${line}`)
        .querySelector('.wb-cart-item__error-text').innerHTML =
  ...
  enableLoading(line) {
    document
      .getElementById('webi-main-cart-items')
      .classList.add('cart__items--disabled')
    this.querySelectorAll(`#WbCartItem-${line} .loading-overlay`).forEach(
